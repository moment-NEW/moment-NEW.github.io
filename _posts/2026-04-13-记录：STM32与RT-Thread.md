...
此外，如果你开启了DMA之类的，Cubemx默认会在msp这个文件里面使用main.c中定义的DMA句柄，但是实际上RT-Thread有自己的一套DMA管理驱动，因此不需要在CubeMX里面开启DMA,否则会报找不到定义的错误（因为cubemx的main.c不能被加进编译链里面）

### ws2812相关
这里不得不吐槽RT-Thread的维护乱象，软件包作者几乎几年没更新了，一些冷门的元件甚至提issue都是石沉大海，email也联系不上。比如这个SEAN_WS2823B，首先不知道这个SEAN何意味，有这个的宏基本上全报错。然后更神秘的是使用了不知道哪个版本的古早SPI API，传入的参数包括GPIOA之类的，由于用不到CS引脚这个玩意不会直接报错而是被算作隐式声明什么的，莫名其妙地被使用进去。
（后记：这是因为RT-Thread有个带cspin的新API和不带cspin的老API）
然后更神秘的是可能由于恰到好处的代码偏移量，这个GPIOA被当成GPIO_PINxx解释之后，长度超过了RT-Thread的限制，因而会被拒绝回收，又因为根本没用到这个引脚，所以不会引发错误。如果你但凡改动一点点代码，改变了这个玩意在内存中的位置，出来的就是莫名其妙的野指针，直接短于RT-Thread的长度限制，导致后续一系列的HAL库Bus Fault，这还是对应的奇怪外设没有使能时钟的情况下，如果使能了会出什么毛病我都不敢想象............
除此之外，这个DM-MC02的ws2812时序也很奇怪，官方例程的SPI是4bit的，频率也不一样，这个RT软件包推荐的是差不多6Mhz，我愣是死活不能输入0X00的黑色RGB，只能让其切换颜色。


最终还是解决了，参考了中科大的驱动，使用了8bit/6.25Mhz的方案。这个多少bit呢，其实是从SPI来的。WS2812使用固定时长内，高电平和低电平的比例驱动的，比如如果被视作逻辑1的话，高电平时间不能少于580ns，不能多于1us，而SPI是根据SCK的时钟周期，在固定的时钟上升下降周期的边沿，修改MOSI的高低电平来实现0/1的发送。由于二者都是1表示高电平，因此我们可以取一段固定的时间，这段时间内。例如我们的8bit方案，一个bit的时间就是1/6.25*10^6=0.16*10^-6=1.6*10^-7,也就是160纳秒。8bit，也就是总共1280ns，我们使用` 0x78U /* 0b01111000 */`来做为spi模拟出的一个ws2812高电平，其中有4个bit是1，也就是4个bit的高电平，4*160=640ns，显然是大于580小于1000的，符合要求。
但是，不知道为什么，理论上4bit，采用`0x0CU /* 0b1100 */`做高电平转换，也就是两个bit，一个bit对应是3.125Mhz，也就是320ns，理论上也是640ns，是完全一致的，却出现了无法黑色化，色彩偏差等各种问题。
### SPI6相关
实际上这个也是BSP/WS2812相关问题的延伸。我在搞WS2812驱动的时候发现了SPI6的配置在H7的RT-thread的驱动中缺失，好巧不巧，这个SPI6的DMA是BDMA，即一次传输一个块的DMA，而且还是低功耗的D3域...也就是只有BDMA这一个DMA外设可供SPI6使用，并且相关驱动还没有人写。
