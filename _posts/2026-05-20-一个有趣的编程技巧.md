...
如你所见，这篇文章里面大部分的玩意都是FOOTGUN，哈哈



## C的可变参数
之前一直使用泛型指针或者结构体和变长数组之类的作为C的可变参数方法，没想到原来本身就有个实现。
这个主要依赖于stdarg这个库。具体来说，以printf为例子，在输入的时候你可以指定若干个参数，但是必须至少有一个固定参数：
```c
printf("helloworld,%d",&words_t);
```
假设我们要自己实现这个printf(不能占用关键字因此加个c_前缀吧)，那这个printf就应该像是：
```c
//这里char[]和char *理论上是一样的，毕竟数组即指针
void c_printf(const char [],...){
    /*阿巴阿巴*/
}
```
其中第一个参数必须是固定的，后续...中就可以是可变参数。
顺便一提，c中没有string，标准库做法是直接把字符数据段压入栈的连续内存内逐字符访问的。此时要引入arg库中的
...
{
    // 第1步：声明一个 va_list 类型的变量（准备一个“指针”）
    va_list args;  

    // 第2步：初始化 va_list，让它指向第一个可变参数
    // 必须传入最后一个固定参数（即 n）作为锚点
    va_start(args, n); 

    double sum = 0.0;
    for (int i = 0; i < n; i++) {
        // 第3步：依次提取可变参数
        // 必须指定要提取的类型（这里为 int），args 会自动向后移动
        int num = va_arg(args, int); 
        sum += num;
    }

    // 第4步：清理收尾，将 va_list 置空（防止野指针）
    va_end(args); 

    return sum / n;
}
```
可以看到这个类似malloc，需要开启和释放。在start的时候会根据你传入的第一个参数类型，计算出起始地址，然后后续根据参数类型大小进行切换。所有参数类型在传入其中的时候都会被进行升格，比如char(大多数平台上uint8_t)会被提升为int(通常是uint16_t);
最后arg指针被释放，生命周期结束，避免出现野指针。


## 短路小特性
在if(!condition1||!condition2)这种判断中，是从左到右的，左边触发了右边就不会触发。

## 一个语法上的「单条语句」
在很多宏中会用到类似ifelse的判断。例如tinyusb的这个：
```c
...
```
本质上就是一个ifelse，但是由于是一个宏，所以可能会出现以下情况：
```c
if (x)
  TU_VERIFY(cond);
else
  do_something();
  ```
为了避免ifelse嵌套或if分号导致的歧义与语法错误，一般用这个do-while(0)循环表示单条语句
# 什么是xfer？
翻阅RT-Thread的驱动时候发现了spi_xfer这种莫名其妙的API命名。询问AI得知，类似cross，trans这种读音相近的词汇会用x简称。xfer即transfer，xmit即transmit。

# refcount
refcount（引用计数） 是一种简单且应用广泛的内存管理技术。它的核心思想是为每个共享资源（如内存对象）维护一个计数器，记录当前有多少个“引用”或“指针”指向它。当引用计数降为0时，意味着没有人在使用该资源，它就可以被安全地释放了。
典型的应用是linux内核的struct kref